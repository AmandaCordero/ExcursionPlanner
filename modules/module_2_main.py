...
class Enviroment:
    ...
    def calculate_dispersion(self):
        ...
        return (self.guide.current_position - firstexc) / 9
    ...
    def regroup(self, point):
        ...
        if self.regroup_count == len(self.excur)+1:
            ...
            env.process(self.guide.move(point, point +1, self.env,self.path))
            for exc in self.excur:
...
class ExcursionAgent:
    ...
    def update_beliefs(self, point_c=None, edge_c=None):
        self.beliefs["point"] = {
            'point_history': 0.7,
            'point_rivers': 0.2,
            'point_flora': 0.8,
...
# Caso de prueba con más puntos en el camino y más excursionistas
...
path.points = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I']
path.size = [3, 4.2,6.5, 4.8, 5.0, 2.5, 4.8, 1.0]  # Distancias entre los puntos
path.slope = [0.5, -0.2, 0.3, -0.5, 0.2, 0.4, -0.3, 0.1]  # Pendientes entre los puntos

# Crear más excursionistas para un caso más grande
guide = GuideAgent(None)  # El entorno aún no se asigna
ex1 = ExcursionAgent("exc1", None)
ex2 = ExcursionAgent("exc2", None)
ex3 = ExcursionAgent("exc3", None)
ex4 = ExcursionAgent("exc4", None)
ex5 = ExcursionAgent("exc5", None)
ex6 = ExcursionAgent("exc6", None)
ex7 = ExcursionAgent("exc7", None)
ex8 = ExcursionAgent("exc8", None)
ex9 = ExcursionAgent("exc9", None)
ex10 = ExcursionAgent("exc10", None)

excursion_agents = [ex1, ex2, ex3, ex4, ex5, ex6, ex7, ex8, ex9, ex10]
environment = Enviroment(guide, excursion_agents, "sunny", path, env)
